...
router.get("/getbrands", queryBuilder, async (req, res) => {
  ...
  try {
    ...
    // Respond with the car details
    return response_handler.okResponse(res, "here you are", { brands: brands, tt: total, total: 0, data_len: 0 })
  } catch (error) {
...
router.get("/getTrimsByModel/:model", queryBuilder, async (req, res) => {
  ...
  try {
    ...
    // Respond with the car details
    return response_handler.okResponse(res, "here you are", { trims: trims, total: total ? total[0].total : 0, data_len: datalen ? datalen[0].data_len : 0 })
  } catch (error) {
    logger.error({ event: "HTTP GET BRANDS ERROR ", error: error?.message })
    response_handler.errorResponse(res, "Server error", error)
  }
