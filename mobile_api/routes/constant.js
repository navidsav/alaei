...
router.get("/getbrands", queryBuilder, async (req, res) => {
  ...
  try {
    ...
    // Respond with the car details
    return response_handler.okResponse(res, "here you are", { brands: brands, total: total.length() > 0 ? total[0].total : 0, data_len: datalen.length() > 0 ? datalen[0].data_len : 0 })
  } catch (error) {
...
router.get("/getModelsByBrand/:brand", queryBuilder, async (req, res) => {
  ...
  try {
    ...
    // Respond with the car details
    return response_handler.okResponse(res, "here you are", { models: models, total: total.length() > 0  ? total[0].total : 0, data_len: datalen.length() > 0  ? datalen[0].data_len : 0 })
  } catch (error) {
    logger.error({ event: "HTTP GET BRANDS ERROR ", error: error?.message })
    response_handler.errorResponse(res, "Server error", error)
  }
...
router.get("/getTrimsByModel/:model", queryBuilder, async (req, res) => {
  ...
  try {
    ...
    // Respond with the car details
    return response_handler.okResponse(res, "here you are", { trims: trims, total: total.length() > 0  ? total[0].total : 0, data_len: datalen.length() > 0  ? datalen[0].data_len : 0 })
  } catch (error) {
    logger.error({ event: "HTTP GET BRANDS ERROR ", error: error?.message })
    response_handler.errorResponse(res, "Server error", error)
  }
...
router.get("/getStatuses", async (req, res) => {
  ...
  return response_handler.okResponse(res, "Here your are", (await loadAdStatus()));
